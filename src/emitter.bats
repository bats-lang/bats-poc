...
fun find_matching_end {ls:agz}{ns:pos}{fuel:nat} .<fuel>.
  ...
  in
    ...
    (* is_boundary: not 97-122, not 65-90, not 48-57, not 95 *)
    val after_ok = ~($AR.gte_int_int(b3, 97) && $AR.lte_int_int(b3, 122)) &&
                   ~($AR.gte_int_int(b3, 65) && $AR.lte_int_int(b3, 90)) &&
                   ~($AR.gte_int_int(b3, 48) && $AR.lte_int_int(b3, 57)) &&
                   ~$AR.eq_int_int(b3, 95)
    val before_ok = ~($AR.gte_int_int(bp, 97) && $AR.lte_int_int(bp, 122)) &&
                    ~($AR.gte_int_int(bp, 65) && $AR.lte_int_int(bp, 90)) &&
                    ~($AR.gte_int_int(bp, 48) && $AR.lte_int_int(bp, 57)) &&
                    ~$AR.eq_int_int(bp, 95)
  in
    (* Check for end: 101,110,100 + boundaries *)
    if $AR.eq_int_int(b0, 101) && $AR.eq_int_int(b1, 110) && $AR.eq_int_int(b2, 100) && after_ok && before_ok then
      (if depth <= 1 then pos
       else find_matching_end(src, pos + 3, src_len, max, depth - 1, fuel - 1))
    (* Check for begin: 98,101,103,105,110 + boundary *)
    else if $AR.eq_int_int(b0, 98) && $AR.eq_int_int(b1, 101) &&
            $AR.eq_int_int(b2, 103) &&
            $AR.eq_int_int($S.borrow_byte(src, pos + 3, max), 105) &&
            $AR.eq_int_int($S.borrow_byte(src, pos + 4, max), 110) && before_ok then
      find_matching_end(src, pos + 5, src_len, max, depth + 1, fuel - 1)
    (* Check for let: 108,101,116 + boundary *)
    else if $AR.eq_int_int(b0, 108) && $AR.eq_int_int(b1, 101) && $AR.eq_int_int(b2, 116) && after_ok && before_ok then
