      ...
      | ~$R.ok(btfd) => let
        ...
        in case+ pr of
          | ~$R.ok(doc) => let
              ...
              (* Read existing bats.lock for version pinning *)
              val el_path = str_to_path_arr("bats.lock")
              val @(fz_elp, bv_elp) = $A.freeze<byte>(el_path)
              val el_or = $F.file_open(bv_elp, 524288, 0, 0)
              val () = $A.drop<byte>(fz_elp, bv_elp)
              val () = $A.free<byte>($A.thaw<byte>(fz_elp))
              val @(el_buf, el_len) = (case+ el_or of
                | ~$R.ok(elfd) => let
                    val eb = $A.alloc<byte>(524288)
                    val er = $F.file_read(elfd, eb, 524288)
                    val elen = (case+ er of | ~$R.ok(n) => n | ~$R.err(_) => 0): int
                    val ecr = $F.file_close(elfd)
                    val () = $R.discard<int><int>(ecr)
                  in @(eb, elen) end
                | ~$R.err(_) => let val eb = $A.alloc<byte>(524288) in @(eb, 0) end
                ): [lel:agz] @($A.arr(byte, lel, 524288), int)
              val @(fz_el, bv_el) = $A.freeze<byte>(el_buf)
              (* Search existing lock for a dep's version.
              ...
              fun lock_find_ver {lel:agz}{lk:agz}{fuel:nat} .<fuel>.
                ...
                in
                  if matches && $AR.eq_int_int(sp, 32) then let
                    ...
                    val vend = find_sp(elb, vstart, 256)
                  in @(vstart, vend) end
                  else let
                    ...
                    val nl = skip_ln(elb, pos, 524288)
                  in lock_find_ver(elb, el_len, ks, ds, de, nl, fuel - 1) end
                end
                ...
                in if dl <= 0 then resolve(ks, de + 1, kl, rb, rl, elb, el_len, lb, cnt, fuel - 1)
                  ...
                  in
                    if lock_vs >= 0 then let
                      ...
                      val () = copy_to_builder_v(elb, lock_vs, lock_ve, 524288, lb)
                      val () = put_char_v(lb, 32)
                      val () = bput_v(lb, "0")
                      val () = put_char_v(lb, 10)
                    in resolve(ks, de+1, kl, rb, rl, elb, el_len, lb, cnt, fuel-1) end
                    else let
                      ...
                      in if blen <= 0 then let
                        ...
                        in resolve(ks, de+1, kl, rb, rl, elb, el_len, lb, cnt, fuel-1) end
                          ...
                          val () = (if already then ()
                              ...
                              val () = bput_v(ua3, "-q")
                              var ua4 = $B.create()
                              val () = copy_to_builder_v(rb, 0, rl, 4096, ua4)
                              val () = bput_v(ua4, "/")
                              val () = arr_range_to_builder_v(ks, dep_start, dep_end, ua4)
                              val () = bput_v(ua4, "/")
                              val () = copy_to_builder_v(bv_b, 0, blen, 256, ua4)
                              var ua5 = $B.create()
                              val () = bput_v(ua5, "-d")
                              var ua6 = $B.create()
                              val () = bput_v(ua6, "bats_modules/")
                              val () = arr_range_to_builder_v(ks, dep_start, dep_end, ua6)
                              val uz_argv = $L.list_vt_cons(mk_arg(ua1),
                                $L.list_vt_cons(mk_arg(ua2), $L.list_vt_cons(mk_arg(ua3),
                                $L.list_vt_cons(mk_arg(ua4), $L.list_vt_cons(mk_arg(ua5),
                                $L.list_vt_cons(mk_arg(ua6), $L.list_vt_nil()))))))
                              val _ = run_cmd(bv_uz, uz_argv)
                        ...
                        in resolve(ks, de+1, kl, rb, rl, elb, el_len, lb, cnt+1, fuel-1) end
                        else let
                          ...
                          val () = $A.drop<byte>(fz_b, bv_b)
                          val () = $A.free<byte>($A.thaw<byte>(fz_b))
                        in resolve(ks, de+1, kl, rb, rl, elb, el_len, lb, cnt+1, fuel-1) end end
                      end
                    | ~$R.err(_) => let
              ...
              val n = resolve(keys, 0, klen, bv_rb3, rplen, bv_el, el_len, lock_b, 0, 200)
              ...
              fun resolve_pass {lr2:agz}{lel2:agz}{fuel:nat} .<fuel>.
                (rb2: !$A.borrow(byte, lr2, 4096), rl2: int,
                 elb2: !$A.borrow(byte, lel2, 524288), el_len2: int,
                ...
                in
                  if new_cnt > prev_cnt then
                    resolve_pass(rb2, rl2, elb2, el_len2, lb2, new_cnt, fuel - 1)
                  else new_cnt
                end
...
fn compile_wasm_runtime(): int = let
  ...
  var wa3 = $B.create() val () = bput_v(wa3, "-O2")
  var wa4 = $B.create() val () = bput_v(wa4, "-nostdlib")
  var wa5 = $B.create() val () = bput_v(wa5, "-ffreestanding")
  var wa6 = $B.create() val () = bput_v(wa6, "-fvisibility=default")
  var wa7 = $B.create() val () = bput_v(wa7, "-D_ATS_CCOMP_HEADER_NONE_")
  var wa8 = $B.create() val () = bput_v(wa8, "-D_ATS_CCOMP_EXCEPTION_NONE_")
  var wa9 = $B.create() val () = bput_v(wa9, "-D_ATS_CCOMP_PRELUDE_NONE_")
  var wa10 = $B.create() val () = bput_v(wa10, "-D_ATS_CCOMP_RUNTIME_NONE_")
  var wa11 = $B.create() val () = bput_v(wa11, "-include")
  var wa12 = $B.create() val () = bput_v(wa12, "build/_bats_wasm_runtime.h")
  var wa13 = $B.create() val () = bput_v(wa13, "-I")
  var wa14 = $B.create() val () = bput_v(wa14, "build/_bats_wasm_stubs")
  var wa15 = $B.create() val () = bput_v(wa15, "-c")
  var wa16 = $B.create() val () = bput_v(wa16, "-o")
  var wa17 = $B.create() val () = bput_v(wa17, "build/_bats_wasm_runtime.wasm.o")
  var wa18 = $B.create() val () = bput_v(wa18, "build/_bats_wasm_runtime.c")
  val argv = $L.list_vt_cons(mk_arg(wa1), $L.list_vt_cons(mk_arg(wa2),
    $L.list_vt_cons(mk_arg(wa3), $L.list_vt_cons(mk_arg(wa4),
    $L.list_vt_cons(mk_arg(wa5), $L.list_vt_cons(mk_arg(wa6),
    $L.list_vt_cons(mk_arg(wa7), $L.list_vt_cons(mk_arg(wa8),
    $L.list_vt_cons(mk_arg(wa9), $L.list_vt_cons(mk_arg(wa10),
    $L.list_vt_cons(mk_arg(wa11), $L.list_vt_cons(mk_arg(wa12),
    $L.list_vt_cons(mk_arg(wa13), $L.list_vt_cons(mk_arg(wa14),
    $L.list_vt_cons(mk_arg(wa15), $L.list_vt_cons(mk_arg(wa16),
    $L.list_vt_cons(mk_arg(wa17), $L.list_vt_cons(mk_arg(wa18),
    $L.list_vt_nil()))))))))))))))))))
  val rc = run_cmd(bv_exec, argv)
  val () = $A.drop<byte>(fz_exec, bv_exec)
  val () = $A.free<byte>($A.thaw<byte>(fz_exec))
...
fn run_wasm_cc {li:agz}{lo:agz}
  ...
  var wc3 = $B.create() val () = bput_v(wc3, "-O2")
  var wc4 = $B.create() val () = bput_v(wc4, "-nostdlib")
  var wc5 = $B.create() val () = bput_v(wc5, "-ffreestanding")
  var wc6 = $B.create() val () = bput_v(wc6, "-fvisibility=default")
  var wc7 = $B.create() val () = bput_v(wc7, "-D_ATS_CCOMP_HEADER_NONE_")
  var wc8 = $B.create() val () = bput_v(wc8, "-D_ATS_CCOMP_EXCEPTION_NONE_")
  var wc9 = $B.create() val () = bput_v(wc9, "-D_ATS_CCOMP_PRELUDE_NONE_")
  var wc10 = $B.create() val () = bput_v(wc10, "-D_ATS_CCOMP_RUNTIME_NONE_")
  var wc11 = $B.create() val () = bput_v(wc11, "-include")
  var wc12 = $B.create() val () = bput_v(wc12, "build/_bats_wasm_runtime.h")
  var wc13 = $B.create() val () = bput_v(wc13, "-I")
  var wc14 = $B.create() val () = bput_v(wc14, "build/_bats_wasm_stubs")
  var wc15 = $B.create() val () = bput_v(wc15, "-Wno-implicit-function-declaration")
  var wc16 = $B.create() val () = bput_v(wc16, "-Wno-int-conversion")
  var wc17 = $B.create() val () = bput_v(wc17, "-c")
  var wc18 = $B.create() val () = bput_v(wc18, "-o")
  var wc19 = $B.create()
  val oc = out_len - 1
  val () = copy_to_builder_v(out_bv, 0, oc, 524288, wc19)
  var wc20 = $B.create()
  val ic = in_len - 1
  val () = copy_to_builder_v(in_bv, 0, ic, 524288, wc20)
  val argv = $L.list_vt_cons(mk_arg(wc1), $L.list_vt_cons(mk_arg(wc2),
    $L.list_vt_cons(mk_arg(wc3), $L.list_vt_cons(mk_arg(wc4),
    $L.list_vt_cons(mk_arg(wc5), $L.list_vt_cons(mk_arg(wc6),
    $L.list_vt_cons(mk_arg(wc7), $L.list_vt_cons(mk_arg(wc8),
    $L.list_vt_cons(mk_arg(wc9), $L.list_vt_cons(mk_arg(wc10),
    $L.list_vt_cons(mk_arg(wc11), $L.list_vt_cons(mk_arg(wc12),
    $L.list_vt_cons(mk_arg(wc13), $L.list_vt_cons(mk_arg(wc14),
    $L.list_vt_cons(mk_arg(wc15), $L.list_vt_cons(mk_arg(wc16),
    $L.list_vt_cons(mk_arg(wc17), $L.list_vt_cons(mk_arg(wc18),
    $L.list_vt_cons(mk_arg(wc19), $L.list_vt_cons(mk_arg(wc20),
    $L.list_vt_nil()))))))))))))))))))))
  val rc = run_cmd(bv_exec, argv)
  val () = $A.drop<byte>(fz_exec, bv_exec)
  val () = $A.free<byte>($A.thaw<byte>(fz_exec))
    ...
    fn ensure_ats2 {l:agz}
      ...
      val rc = (if exists then 0
        ...
        val make_argv = $L.list_vt_cons(mk_arg(mk1), $L.list_vt_cons(mk_arg(mk2),
          ...
          $L.list_vt_nil()))))))
        val r3 = run_cmd(bv_me, make_argv)
