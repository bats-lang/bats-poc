                              ...
                              in
                                ...
                                in
                                  ...
                                  in scan_ns(nsd2, ph2, ns_bv, ns_len, fuel2 - 1) end
                                    ...
                                    val mc2 = $B.create()
                                    val () = bput(mc2, "build/bats_modules/")
                                          ...
                                          fun patsopt_ns {lph2:agz}{lns2:agz}{fuel_ns:nat} .<fuel_ns>.
                                            ...
                                            in if sel < 0 then $A.free<byte>(sde)
                                                ...
                                                in patsopt_ns(nsd, ph2, ph2len, ns_name, ns_len, fuel_ns-1) end
                                                  ...
                                                  val po = $B.create()
                                                  val () = bput(po, "build/bats_modules/")
                                          ...
                                          fun clang_ns {lph3:agz}{lns3:agz}{fuel_cn:nat} .<fuel_cn>.
                                            ...
                                            in if sel < 0 then $A.free<byte>(sde)
                                                ...
                                                in clang_ns(nscd, ph3, ph3len, ns3, ns3len, rr3, fuel_cn-1) end
                                                  ...
                                                  val @(fz_sde, bv_sde) = $A.freeze<byte>(sde)
                                                  val co = $B.create()
                                                  val () = bput(co, "build/bats_modules/")
          ...
          | ~$R.ok(tcfd) => let
            ...
            in
              if tcl2 > 0 then let
                ...
                val cp_argv = $B.create()
                val () = bput(cp_argv, "cp") val () = $B.put_byte(cp_argv, 0)
                val () = bput(cp_argv, "-r") val () = $B.put_byte(cp_argv, 0)
                val () = bput(cp_argv, "build/.") val () = $B.put_byte(cp_argv, 0)
                val () = copy_to_builder(bv_tcb, 0, tcl2, 4096, cp_argv, $AR.checked_nat(tcl2+1))
                val () = bput(cp_argv, "/") val () = $B.put_byte(cp_argv, 0)
                val _ = run_cmd(bv_cpe, 524288, cp_argv, 4)
                val () = $A.drop<byte>(fz_cpe, bv_cpe)
                val () = $A.free<byte>($A.thaw<byte>(fz_cpe))
                (* Generate Makefile *)
                val mf = $B.create()
                val () = bput(mf, "PATSHOME ?= $(HOME)/.bats/ats2\n")
                val () = bput(mf, "CC ?= cc\n\n")
                val () = bput(mf, "%_debug.o: %.c\n\t$(CC) -c -o $@ $< -g -O0 -I$(PATSHOME) -I$(PATSHOME)/ccomp/runtime\n\n")
                val () = bput(mf, "%_release.o: %.c\n\t$(CC) -c -o $@ $< -O2 -I$(PATSHOME) -I$(PATSHOME)/ccomp/runtime\n\n")
                val () = bput(mf, "SRCS := $(shell find . -name '*_dats.c') _bats_native_runtime.c\n")
                val () = bput(mf, "DEBUG_OBJS := $(SRCS:.c=_debug.o)\n")
                val () = bput(mf, "RELEASE_OBJS := $(SRCS:.c=_release.o)\n\n")
                val () = bput(mf, "all: debug/bats release/bats\n\n")
                val () = bput(mf, "debug/bats: $(DEBUG_OBJS)\n\tmkdir -p debug\n\t$(CC) -o $@ $^ -g -O0\n\n")
                val () = bput(mf, "release/bats: $(RELEASE_OBJS)\n\tmkdir -p release\n\t$(CC) -o $@ $^ -O2\n\n")
                val () = bput(mf, "clean:\n\trm -f *.o\n\trm -rf debug release\n")
                val mfp = $B.create()
                val () = copy_to_builder(bv_tcb, 0, tcl2, 4096, mfp, $AR.checked_nat(tcl2+1))
                val () = bput(mfp, "/Makefile")
